...
export class SzerkesztesComponent {
  ...
  removeKategoria() {
    ...
    let modal = this.modalService.open(ModaldeleteComponent, {backdrop:'static', centered: true});
    (modal.componentInstance as ModaldeleteComponent)
    .initModalDeleteWindow({receivedelement: katnev, routefrommodal : '/szerkesztes'}, {del: () => {modal.close(); this.apiService.deleteKategoria(this.authService.getToken(), this.kategoriaTodelete).subscribe(response => {
      console.log(response);
      //this.kategoriak = this.kategoriak.filter(k => k.id !== Number(this.kategoriaTodelete));
      this.ngOnInit();
    })}, cancel: () => {modal.close();}});
  }
}
